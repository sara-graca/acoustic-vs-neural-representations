...
# Phonological ground-truth partitions for vowels
FRONTBACK = {
...
print(f"Top {N_TOP_CONSONANTS} consonants: {top_cons}")
...
# Combined vowel + consonant subset
df_vc = pd.concat([df_v, df_c], ignore_index=True)
...
# For combined: need to stack vowel + consonant rows in original df order
...
# 3. Compute per-phoneme mean vectors


def phoneme_centroids_acoustic(df_sub: pd.DataFrame,
                                phones: list,
                                features: list) -> tuple[np.ndarray, list]:
    """Returns (centroid matrix, valid phone list)."""
    rows, valid = [], []
    for p in phones:
...
def phoneme_centroids_neural(X: np.ndarray,
                              labels: np.ndarray,
...
def ward_cluster(centroids: np.ndarray,
                 metric: str = "euclidean") -> np.ndarray:
    """Ward linkage matrix."""
    if metric == "cosine":
        D = squareform(pdist(centroids, metric="cosine"))
        # Ward requires Euclidean — use average linkage for cosine
...
def best_k(centroids: np.ndarray,
           Z: np.ndarray,
...
def ari_evaluation(Z: np.ndarray,
                   phones: list,
                   partition: dict,
                   k: int) -> float:
    """ARI between cluster assignments and ground-truth partition."""
...
def plot_dendrogram(Z: np.ndarray,
                    ...
                    title: str,
                    out_path: str,
                    color_partition: dict = None):
    fig, ax = plt.subplots(figsize=(10, 5))
    dendrogram(Z, labels=labels, ax=ax, leaf_rotation=0,
               orientation="top")

    if color_partition:
        unique_groups = list(set(color_partition.values()))
...
# ── 5a. Acoustic vowel clustering ──
C_ac, phones_ac = phoneme_centroids_acoustic(
    df_v, VOWELS, ["F1_norm", "F2_norm"]
)
scaler = StandardScaler()
C_ac_scaled = scaler.fit_transform(C_ac)
...
print(f"Acoustic vowels — best k={k_ac}, silhouette={sil_ac:.3f}")
print(f"  ARI front/back: {ari_fb_ac}, ARI height: {ari_ht_ac}")

ari_rows.append({
    "analysis": "vowel", "representation": "acoustic",
...
plot_dendrogram(
    Z_ac, phones_ac,
    title="Vowel clustering — Acoustic (Ward, Euclidean)",
    out_path="results/figures/cluster_vowel_acoustic.png",
    color_partition=HEIGHT,
)

# Acoustic vowel clustering — F1 only
C_ac_f1, phones_ac_f1 = phoneme_centroids_acoustic(
    df_v, VOWELS, ["F1_norm"]
)
...
for model_name, X in neural_vowel.items():
    ...
    print(f"{model_name} vowels — best k={k_ne}, silhouette={sil_ne:.3f}")
...
# 6. Section 9.2 — Consonant extension


print("\n=== Section 9.2: Vowel + consonant clustering ===")

# Ground truth: C/V boundary
CV_PARTITION = {p: "vowel" for p in VOWELS}
CV_PARTITION.update({p: "consonant" for p in top_cons})

# Manner of articulation for consonants (French)
MANNER = {
    "w": "approximant", "ʁ": "approximant", "l": "approximant",
    "t": "stop",  "d": "stop",  "p": "stop",  "k": "stop",
    "f": "fricative", "s": "fricative", "ʃ": "fricative",
}
MANNER.update({p: HEIGHT.get(p, "vowel") for p in VOWELS})

# ── 6a. Acoustic V+C clustering ──
# For consonants use duration + scg; for vowels use F1_norm + F2_norm
# Build separate feature vectors and concatenate after standardising

def build_vc_acoustic_centroids(df_v_sub, df_c_sub, vowels, consonants):
    rows, valid = [], []

    # Vowels: F1_norm, F2_norm, duration_ms (pad scg with mean)
    ...
    for p in vowels:
        ...
        if len(sub) < 2:
            continue
        m = sub.mean().values
        rows.append(np.append(m, scg_mean))  # pad scg
        valid.append(p)

    # Consonants: duration_ms + scg (pad F1/F2 with vowel means)
    f1_mean = df_v_sub["F1_norm"].mean()
    f2_mean = df_v_sub["F2_norm"].mean()
    dur_mean = df_v_sub["duration_ms"].mean()
    for p in consonants:
...
C_vc_ac, phones_vc_ac = build_vc_acoustic_centroids(
    df_v, df_c, VOWELS, top_cons
)
C_vc_ac_scaled = StandardScaler().fit_transform(C_vc_ac)
Z_vc_ac = ward_cluster(C_vc_ac_scaled, metric="euclidean")
k_vc_ac, sil_vc_ac = best_k(C_vc_ac_scaled, Z_vc_ac,
                              k_range=range(2, 8))
...
ari_rows.append({
    ...
    "ARI_frontback": "", "ARI_height": "",
    "ARI_CV": ari_cv_ac,
})

plot_dendrogram(
    Z_vc_ac, phones_vc_ac,
...
# ── 6b. Neural V+C clustering ──
labels_vc = np.array(
    df_v["phoneme"].tolist() + df_c["phoneme"].tolist()
)

for model_name, X in neural_vc.items():
    C_vc_ne, phones_vc_ne = phoneme_centroids_neural(
        X, labels_vc, PHONES_VC
    )
